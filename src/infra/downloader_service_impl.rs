...
use crate::services::downloader_service::{DownloaderService, DownloadTaskResp, SubTaskResp};
use crate::models::downloader::{DownloadTask, CreateDownloadReq, ResolveMagnetReq, ResolveMagnetResp, StartMagnetDownloadReq, TorrentFileMetadata};
use chrono::Utc;
use uuid::Uuid;
...
impl DownloaderService for DownloaderServiceImpl {
    ...
    async fn resolve_magnet(&self, req: ResolveMagnetReq) -> Result<ResolveMagnetResp> {
        ...
        match add_result {
            AddTorrentResponse::ListOnly(resp) => {
                ...
                {
                    let mut cache = self.magnet_cache.lock().await;
    ...
    async fn start_magnet_download(&self, username: &str, req: StartMagnetDownloadReq) -> Result<()> {
        let torrent_bytes = {
            let cache = self.magnet_cache.lock().await;
            match cache.get(&req.token) {
                Some(b) => b.clone(),
                None => return Err(AppError::BadRequest("Token expired or invalid".to_string())),
            }
        };
