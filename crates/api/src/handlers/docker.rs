...
use domain::dtos::container::{IdReq, CreateContainerReq, PullImageReq, CreateVolumeReq, RemoveVolumeReq};
...
pub async fn create_volume(State(st): State<AppState>, Json(req): Json<CreateVolumeReq>) -> ApiResult<impl IntoResponse> {
    st.container_service.create_volume(req).await?;
    Ok(Json(serde_json::json!({ "ok": true })))
}

pub async fn remove_volume(State(st): State<AppState>, Json(req): Json<RemoveVolumeReq>) -> ApiResult<impl IntoResponse> {
    st.container_service.remove_volume(&req.name).await?;
    Ok(Json(serde_json::json!({ "ok": true })))
}
